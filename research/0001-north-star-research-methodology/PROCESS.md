...
> **The test branch did not merge, and it may be deleted.** The result was negative, so none
> of that code belongs on `main`. Everything needed to read or repeat the test is therefore
> copied into [`data/`](data/): the runner, the scorer, the lane driver, the treatment
> doctrine, and its diff against the control. **This directory is self-contained.** Nothing
> below depends on the test branch still existing.

## 2. The two arms

Both arms run the same pipeline. Only one file differs.

| Arm | File used as `doctrine/roles/technical-research.md` | sha256 | Preserved at |
|---|---|---|---|
| A (control) | `doctrine/roles/technical-research.md` | `88f093f8…7f8ac3ad` | `git show 42a88eb:doctrine/roles/technical-research.md` |
| B (treatment) | `doctrine/roles/technical-research.northstar.md` | `28510098…ccebb834` | [`data/treatment-doctrine.md`](data/treatment-doctrine.md) |

The control lives on `main`, so git preserves it. The treatment lived only on the test branch,
so this directory keeps a copy.

The treatment changes **55 lines in 4 hunks**. The exact change is
[`data/treatment.diff`](data/treatment.diff). The runner copies the treatment file over the
...
**Runner** — [`data/run-ab.sh`](data/run-ab.sh), invoked as `run-ab.sh <slug> <a|b>`. It sat at
`tools/experiment/run-ab.sh` on the test branch.
...
**Scorer** — [`data/compare-arms.mjs`](data/compare-arms.mjs), invoked as
`compare-arms.mjs <slug>`. It reports cost per phase, the user interface labels, the structure
of each file, and the process record. **This tool has a defect. See section 7.**
...
To repeat the test, copy the tools out of `data/` into a working tree, restore the usage
instrumentation (see below), and run:

```bash
D=research/0001-north-star-research-methodology/data

# one cell
mise exec -- "$D/run-ab.sh" google-sheets a

# the whole matrix, three lanes
mise exec -- bash "$D/matrix.sh"

# score one provider
node "$D/compare-arms.mjs" google-sheets
```
...
The copy in [`data/compare-arms.mjs`](data/compare-arms.mjs) still has this defect; it is
preserved as it ran, not as it should be. Before anyone reuses it, it must average the
