...
| Redirect URI | Non-legacy remote-session authorization uses the configured server origin plus `/mcp/remote_login_callback`. Clients marked `LegacyCallbackUrl` instead send `/oauth/callback` to preserve the provider’s registered redirect URI. DCR registers `/oauth/callback`, `/mcp/remote_login_callback`, and `/x/mcp/remote_login_callback`; the legacy OAuth callback forwards to the canonical callback. Use the documented/displayed URI for the relevant setup surface, not an invented hostname or localhost callback. | [challenge.go], `callbackURL`, `HandleLegacyProxyCallback`; [proxyregister.go] |
