...
For every complete concurrent review wave, project each reviewer result inside that same compose program before returning the wave. The projection for each reviewer contains only its `output` plus a minimal reusable session handle containing `id` and `generation`; return the linter result alongside those projections. Never return reviewer `updates`, complete transport envelopes, prompts, or session history. If malformed output requires the one permitted repair, reconstruct the prompt tool's session value from the projected `id`, `generation`, and `output`; set `updates` to exactly `{items:[],truncated:false}` and pass the same `factory/schemas/review-findings.schema.json` as `output_schema`. After the prompt succeeds, project the repaired result with its new `generation` through the same bounded projection. Never use a local parser, shell, jq, Python, or another tool to recover review-wave results.
