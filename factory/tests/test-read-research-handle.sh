...
reject 1 1
jq -n '{id:"session-1",name:"topic-1",generation:1,output:("x" * 36000),updates:{items:[{opaque:{extra:[1,2,3],data:("u" * 70000)}}],truncated:false}}' >"$base-0.handle.json"
printf '{"topic_id":1}' >"$base-0.input.json"
chmod 600 "$base-0."*.json
[[ $(wc -c <"$base-0.handle.json") -gt 65536 ]]
read_handle 1 1 >"$tmp/out"; cmp "$base-0.handle.json" "$tmp/out"
jq '.generation=2 | .updates.items += [{new:{opaque:("v" * 70000)}}]' "$base-0.handle.json" >"$base-1.handle.json"
printf '{"topic_id":1,"follow_up_index":1}' >"$base-1.input.json"
chmod 600 "$base-1."*.json
[[ $(wc -c <"$base-1.handle.json") -gt $(wc -c <"$base-0.handle.json") ]]
...
printf '{"topic_id":1,"follow_up_index":1}' >"$base-1.input.json"
jq '.output=("x" * 1048577)' "$tmp/good" >"$base-1.handle.json"; reject 1 2
cp "$tmp/good" "$base-1.handle.json"
