...
**Host identity extension (Task 5):** Finalization version 1 additionally carries `host_run_id` (32 lowercase hex), `workflow_run_id` (string), and `workflow_run_attempt` (integer). The current workflow shell generates `FACTORY_HOST_RUN_ID` independently before launch and exports it through host-only `GITHUB_ENV`; run-kit accepts that explicit identity or generates a fresh local identity. The supervisor supplies its validated invocation ID and inherited workflow identity to its host-only finalizer environment, never Docker/provider environment. Local finalization uses empty workflow ID/zero attempt; publication still requires explicit current host ID plus trusted GitHub run/attempt/upload inputs. Promotion and publisher reject mismatches rather than deriving expectations from exported files. This extends only host-owned finalization, not model report schemas or publication receipt fields.
